...
export default function BookingWidget({ property }: BookingWidgetProps) {
    ...
    const handleBooking = () => {
        ...
        toast({
            ...
            description: `You've booked ${property.title} for ${guests} guest(s) from ${format(dateRange.from, "PPP")} to ${format(
                dateRange.to,
                "PPP"
                ...
                {totalPrice !== null && dateRange.from && dateRange.to && (
                    <div className="text-center text-sm space-y-1">
                        <div className="flex justify-between">
                            <span>
                                ₹{property.pricePerNight.toLocaleString("en-IN")} x {differenceInDays(dateRange.to, dateRange.from)} nights
                            </span>
                            <span>
                                ₹{(property.pricePerNight * differenceInDays(dateRange.to, dateRange.from)).toLocaleString("en-IN")}
                            </span>
                        </div>
                        <div className="flex justify-between">
